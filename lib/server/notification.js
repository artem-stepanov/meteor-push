...
const sendNotification = (fcmConnections, defaults, userToken, mongoNote) => {
  ...
  const apns = {
    ...
    // },
    payload: {
      // "Simulator Target Bundle": "com.artemstepanov.myforward",
      // "gcm.message_id": userToken,
  ...
  // eslint-disable-next-line camelcase
  // const note = { android, apns, webpush, fcm_options, data }
  const note = { android, apns, data };
  ...
  if (isDebug) {
    console.log("Final notification right before shoot out:", note);
  }
  ...
  fcmConnections.send(note)
    ...
    .catch(error => {
      if (isDebug) {
        console.log("FCM Sending Error: ", error?.message);
      }
    });
};
