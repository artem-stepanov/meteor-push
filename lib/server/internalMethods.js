...
if (process.env.SERVER_IS_PUSH_SAVER === 'true') {
  Meteor.methods({
    ...
    'token-validate': async function ({ _id, token }) {
      console.log('token-validate', { _id, token })
      ...
      try {
        ...
        const document = await Push.appCollection.findOneAsync({ _id }, { fields: { tokens: 1, userId: 1 } })
        
        if (!document) {
          console.log('Document not found for _id:', _id)
          return false
        }

        // Если пользователь авторизован, проверяем что документ принадлежит ему
        if (this.userId && document.userId !== this.userId) {
          console.log('Document belongs to different user')
          return false
        }

        // Проверяем есть ли указанный токен в массиве tokens
        const tokenExists = document.tokens && document.tokens.some(t => t.token === token)
        
        console.log('Token validation result:', tokenExists)
        return tokenExists
      } catch (error) {
